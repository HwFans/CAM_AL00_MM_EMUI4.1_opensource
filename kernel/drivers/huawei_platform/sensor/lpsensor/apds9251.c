...
static long lux_mix = 0;     /* A and daylight mix */ 
static long avago_cofficient[4] = {27, 18, 10,5};
...
static int LuxCalculation(struct i2c_client *client)
{
	...
	if (ir_r_ratio > avago_cofficient[0]){
		...
		if(lux_mix==LUX_MIX &&  b_r_ratio > avago_cofficient[3]){
...
static int tp_color_to_id(int color)
{
	...
	case GOLD_TP:
		...
		break;
	default:
		APDS9251_ERR("%s:read tp color failed,color = %d\n",__func__,color);
		color_id = WHITE;
		break;
	}
	return color_id;
...
static int parse_tp_color_and_module_manufacture(struct rgb_apds9251_data *data)
{
	...
	for (i = 0;i < MODULE_MANUFACTURE_NUMBER;i++){
		if (pdata->panel_id == apds9251_tp_module_parameter[i].tp_module_id){
			if (pdata->tp_color == GOLD){
				...
				avago_cofficient[3] = apds9251_tp_module_parameter[i].gold_apds9251_lux_cal_parameter.avago_cofficient[3];
				lux_mix = apds9251_tp_module_parameter[i].gold_apds9251_lux_cal_parameter.a_daylight_mix;
				...
				avago_cofficient[3] = apds9251_tp_module_parameter[i].white_apds9251_lux_cal_parameter.avago_cofficient[3];
				lux_mix = apds9251_tp_module_parameter[i].white_apds9251_lux_cal_parameter.a_daylight_mix;
				...
				avago_cofficient[3] = apds9251_tp_module_parameter[i].black_apds9251_lux_cal_parameter.avago_cofficient[3];
				lux_mix = apds9251_tp_module_parameter[i].black_apds9251_lux_cal_parameter.a_daylight_mix;
				...
				avago_cofficient[3] = apds9251_tp_module_parameter[i].blue_apds9251_lux_cal_parameter.avago_cofficient[3];
				lux_mix = apds9251_tp_module_parameter[i].blue_apds9251_lux_cal_parameter.a_daylight_mix;
			}
			else
			{
				APDS9251_ERR("%s:%d:tp_type =%d,get tp_type is fail.\n",__FUNCTION__, __LINE__,pdata->panel_id);
	...
	APDS9251_INFO("%s: avago_cofficient[3][%ld],lux_mix[%ld]\n",__func__, avago_cofficient[3],lux_mix);
	return SUCCESE_ALS;
}
