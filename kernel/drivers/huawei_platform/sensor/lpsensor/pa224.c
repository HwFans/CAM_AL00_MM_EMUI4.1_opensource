...
static unsigned int threshold_value = 21; /*the value of min far ps*/
...
static void txc_pa2240_ps_report_event(struct i2c_client *client)
{
	...
	if(data->ps_data <= data->pilt){
		...
		if((data->oil_occur) || (far_ps_min > data->ps_data)){
			far_ps_min = data->ps_data < threshold_value ? threshold_value : data->ps_data;
		...
		struct txc_pa2240_platform_data *pdata)
{
	...
	rc = of_property_read_u32(np, "txc,threshold_value", &tmp);
	if (rc) {
		TXC_PA2240_ERR("%s,line %d:Unable to read threshold_value\n",__func__,__LINE__);
		return rc;
	}
	threshold_value = tmp;
